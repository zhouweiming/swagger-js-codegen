...
var getViewForSwagger2 = function (opts, type) {
    ...
    var data = {
        ...
        methods: [],
        requestHelper: opts.requestHelper,
        // apis: [],
        definitions: []
    };
    ...
    _.forEach(swagger.paths, function (api, path) {
        ...
        _.forEach(api, function (op, m) {
            ...
            // console.info(apiMap[methodName].param);
            var method = {
                path: path,
                className: opts.className,
                methodName: methodName,
                parameterConfig: JSON.stringify(apiMap[methodName] && apiMap[methodName].param || {}),
                method: M,
                isGET: M === 'GET',
                isPOST: M === 'POST',
            ...
            _.forEach(op.responses, function (val, key) {
                if (key.startsWith('2')) {
                    if (val.schema) {
                        ...
                        if (method.methodTsType.properties) {
                            const dataType = method.methodTsType.properties.filter(item => item.name === 'data');
